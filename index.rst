...
.. note::
   ...
   having different levels (eg software developer and senior software
   developer) make it harder
...
.. note::
   ...
   The salary rates are (or soon will be) based on average salaries, per
   institution.
...
.. note::
  ...
  Perhaps mention science time is charged to KLM20201? (I haven't checked
  if that is the case for every science person, but we can easily find out)
...
.. note::
  ...
  Maybe mention it is a an estimate, and it is normal to have variations,
  eg in December when there is long period of holidays it is much fewer
  SPs/developer, so other months need to compensate for that
...
.. note::
   ...
   Indeed. I think my intention was to say that a planning package never
   spans 3rd WBS level. But I guess they don't even span 4th.
...
.. note::
   ...
   No, level 1 milestones are exposed to NSF. NSF picked milestones they
   wanted to see, making them level 1. ProjMgr should weight in on level 2
   because they involve other subsystems. For level 3, if a milestone is
   related to say team x blocking team y, the tcams of these two teams
   should decide.
...
A planning package has a well defined scope, deliveration(s) a duration and
a staff assignment (it is “resource
...
:: note..
  ...
  Cycle is not defined, it is introduced for the very first time above
...
In F17, both members of staff are assigned to a 6-month epic derived from ``P``. The
...
The :ref:`Project Controls Specialist <sec-contacts>` will periodically
automatically pull
...
.. note::
   ...
   work, sketching plans for future cycles, etc: please feel free to define them as
   necessary.

.. note::

   The above note needs to be fixed. Epics that do not satisfy some criteria
   (cycle, wbs) will not be pulled into PMCS. But you can have epics that do not
   satisfy some criteria (like quality of description) and they will still be
   pulled
...
.. note::
   ...
   Might be worth mentioning above that primavera breaks epics into equal
   per-month chunks.
...
.. note::
   ...
   estimate, description and deliverables though.
...
Assuming everything has gone according to plan, by the end of a cycle deliverables
for all epics should be verified, and all epics should be marked as “done”.
When an epic is marked as done, it is equivalent to
...
day-to-day work. Exception: very small epics (few SPs). Rather, each epic is broken down into a series of
self-contained “stories”. A :term:`story` describes a planned activity of a
single developer worth
...
for the schedule and cost variance. See :ref:`sec-cycle-value` for its impact on earned value.
...
.. note::
   ...
   No, we don't require a sprint to be a month long.
...
#. Preparation.
   ...
   Do not overload developers. Take vacations and holidays into the account.
   The sprint should describe a plausible amount of work for the time available.
...
#. Execution.
   ...
   up-to-date with reality; if necessary, already scheduled stories may be pushed
   out of a sprint as soon as it is obvious it is unrealistic to expect them
   to be completed as part of a given sprint.
...
and software maintenance: any more than this must be carefully justified.
...
should be submitted by the 10th of the month
(refer to the :ref:`sec-monthly-cycle`). You are encouraged to submit your report as early in the month as
possible.
...
month being reported on and plans for the upcoming month, brief (1-2 sentences)
high level summary for the Overall Summary, as well as
