...
However, as above we generally aspire to minimize the fraction of \gls{loe}; T/CAMs are encouraged to explicitly schedule as much developer time as is possible.
...
In early development, releases followed a strictly time-based cadence. That is, they are
made on a pre-defined schedule which tracks our
short-term plan (\S\ref{sec:cycle-plan}), rather than
being guaranteed to provide a particular set of functionality. 
As of F17 with the test schedule in \citeds{LDM-503} release will have to provide required functionality for those tests.
Still there is no need for individual releases will not be exposed as milestones above
