...
This is summarized in tab-working-rate.
...
Planning packages are defined at the fourth level of the \gls{wbs} breakdown
(e.g. at 1.02C.04.02; see \S\ref{sec:wbs}). They may not cut
across the \gls{wbs} structure, but rather must refer to that particular
fourth-level \gls{element} and its children.
...
\item
  The :jira\_field:cycle field set to the appropriate \gls{cycle};
\item
  The :jira\_field:\gls{wbs} field set to the appropriate \gls{wbs} \emph{leaf}
  \gls{element}.
\item
  The :jira\_field:Story Points field set to a (non-zero!) estimate of the effort required to complete the \gls{epic} in terms of \glspl{sp} (see \S\ref{sec:effort}).
\end{itemize}
...
Although it is possible---indeed, encouraged---to set the :jira\_field:assignee field in \gls{jira} to the individual who is expected to carry out the bulk of the work in an \gls{epic}, this does not provide sufficient granularity for those cases when more than one person will be contributing.
...
Assuming everything has gone to plan, by the end of a \gls{cycle} all deliverables should be verified and the corresponding \glspl{epic} should be marked as :jira\_field:done.
Marking an \gls{epic} as :jira\_field:done asserts that the concrete deliverable associated with the \gls{epic} has been provided.
...
An \gls{epic} may be marked as :jira\_field:done when:
...
The three issue types are functionally equivalent: these semantic
...
data which is loaded to the \gls{pmcs}: it makes no distinction between them.
...
In other cases, it can be deferred to a later \gls{cycle}, or, after appropriate discussion, may be regarded as inappropriate (and can be tagged as :jira\_field:invalid or :jira\_field:won't fix).
...
Please close these as :jira\_field:invalid, and add a \gls{jira} link of type :jira\_field:duplicates to the original ticket.
...
Be aware that stories that marked as :jira\_field:invalid or :jira\_field:won't fix in \gls{jira} are not included in this calculation: they earn no value.
